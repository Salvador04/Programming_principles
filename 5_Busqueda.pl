...
print "Escribe la longitud del conjunto (debe estar en el rango entre 1 a 10000):\n";
...
while ($Inicio <= $Final) {
	...
	if ($Numero == $Conjunto [$Mitad]) {
	  print "El indice del conjunto donde se encuentra el numero es: " . int($Mitad) . "\n";
    last;
	}
  elsif ($Numero < $Conjunto [$Mitad]) {
	  $Final = $Mitad - 1;
	}
  else {
	  $Inicio = $Mitad + 1;
